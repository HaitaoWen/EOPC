...
from . import distance as distance_lib
from . import utils
...
class CosineClassifier(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(scaling, int) or isinstance(scaling, float):
            ...
            printlog("Using inner learned scaling")
            self.scaling = FactorScalar(1.)

        if proxy_per_class > 1:
            printlog("Using {} proxies per class.".format(proxy_per_class))
    ...
    @property
    def weights(self):
        if hasattr(self._weights, '_former_parameters'):
            return torch.cat(list(self._weights._former_parameters.values()))
        else:
    ...
    def add_imprinted_classes(
        ...
        if self.proxy_per_class > 1:
            printlog("Multi class diff {}.".format(multi_class_diff))
        ...
        for class_index in class_indexes:
            mask = class_index == inc_dataset._y
            images = inc_dataset._x[mask]
            imagesdata = Images(images, inc_dataset.trsf)
            if args.mode == 'DDP':
