...
def extract_features(model, loader):
    ...
    for x, y in loader:
        _targets = y.numpy()
        _features = model.extract(x.to(model.classifier.weights.device)).detach().cpu().numpy()
...
def add_new_weights(network, weight_generation, current_nb_classes, task_size, inc_dataset):
    ...
    if weight_generation["type"] == "imprinted":
        printlog("Generating imprinted weights")
        ...
        printlog("Generating embedding weights")
